...
const PostHiringTable = () => {
  ...
  const fetchJobPosts = async () => {
    try {
      ...
      console.error("Error fetching post hiring:", {
  ...
  const handleArchiveConfirm = async () => {
    ...
    try {
      ...
      message.success(`Post hiring for "${companies[postToArchive.company_id]?.company_name || "N/A"}" archived successfully`);
      setIsConfirmModalOpen(false);
      setPostToArchive(null);
      fetchJobPosts(); // Refresh data after archiving
    } catch (error) {
      console.error("Error archiving post hiring:", error);
      message.error(error.response?.data?.message || "Failed to archive post hiring.");
      setError(error.response?.data?.message || "Failed to archive post hiring.");
    }
  };
  ...
  const handleRestorePost = async (postId) => {
    try {
      ...
      message.success("Post hiring restored successfully");
      fetchJobPosts(); // Refresh data after restoring
    } catch (error) {
      console.error("Error restoring post hiring:", error);
      message.error(error.response?.data?.message || "Failed to restore post hiring.");
      setError(error.response?.data?.message || "Failed to restore post hiring.");
    }
  };

  const handleBulkAction = async (action) => {
    if (selectedPosts.length === 0) {
      message.warning("No post hiring selected.");
      return;
    }
    try {
      ...
      message.success(`Selected post hiring ${action === "archive" ? "archived" : "restored"} successfully`);
      setSelectedPosts([]);
      fetchJobPosts(); // Refresh data after bulk action
    } catch (error) {
      console.error(`Error performing bulk ${action}:`, error);
      message.error(error.response?.data?.message || `Failed to ${action} post hiring.`);
      setError(error.response?.data?.message || `Failed to ${action} post hiring.`);
    }
  };
  ...
  const handlePostAdd = async (newPost) => {
    try {
      ...
      message.success("Post hiring created successfully");
      setIsModalOpen(false);
      fetchJobPosts(); // Refresh data after adding
    } catch (error) {
      console.error("Error adding post hiring:", error);
      message.error(error.response?.data?.message || "Failed to create post hiring.");
      throw error;
    }
  };
  ...
  const handlePostUpdate = async (updatedPost) => {
    try {
      ...
      message.success("Post hiring updated successfully");
      setIsModalOpen(false);
      setIsEditMode(false);
      setPostToEdit(null);
      fetchJobPosts(); // Refresh data after updating
    } catch (error) {
      console.error("Error updating post hiring:", error);
      message.error(error.response?.data?.message || "Failed to update post hiring.");
      throw error;
    }
  };
  ...
  return (
    <div className="app">
      ...
      <div className="posthiring-dashboard">
        <div className="posthiring-content">
          <h2>{showArchived ? "Archived Post Hiring" : "Post Hiring"}</h2>
          {error && <div className="error-message">{error}</div>}
          <div className="posthiring-header">
          ...
          <ErrorBoundary>
            <div className="posthiring-table">
              <table>
                <thead>
                  <tr>
                    <th>
                ...
                <tbody>
                  {filteredPosts.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="19">No {showArchived ? "archived" : "active"} post hiring found</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="posthiring-pagination">
      ...
      {isConfirmModalOpen && (
        <div className="confirm-modal-overlay">
          <div className="confirm-modal">
            <h3>Are you sure?</h3>
            <p>Do you want to archive post hiring for "{companies[postToArchive?.company_id]?.company_name || "N/A"}"?</p>
...
export default PostHiringTable;
