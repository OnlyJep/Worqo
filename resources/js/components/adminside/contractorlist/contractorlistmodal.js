...
const ContractorModal = ({ onClose, onSubmit, isEdit, initialData, genders, suffixes }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.errors) {
        ...
        const errorMsg = error.response?.data?.error || error.response?.data?.message || error.message || "An error occurred. Please try again.";
  ...
  return (
    <div className="employermodal-overlay">
      <div className="employermodal">
        <h2>{isEdit ? "Edit Contractor" : "Add New Contractor"}</h2>
